(function(angular) {
	...
	.controller( 'xpsui:PushNotificationCtrl', [
		'$scope',
		'$routeParams',
        ...
        angular.forEach($scope.userList, function (obj) {
			...
			$scope.search = function() {
				...
				genericSearchFactory.getSearch(entityUri, c,convertSortBy( $scope.sortBy),0,pageSize).success(function(data) {
					...
					for(i = 0; i < data.length; i++) {
						if(data[i].pushregid) {
							$scope.userList.push(data[i]); // Add only data with a push notification registration id.
						}
					}
			...
			$scope.send = function() {
				...
				genericSearchFactory.getSearch(entityUri, c,convertSortBy( $scope.sortBy),0,pageSize).success(function(data) {
					var pushregids = new Array();
					for(var i = 0; i < $scope.userList.length; i++) {
						if($scope.userList[i].pushregid && document.getElementById(i).checked) {
