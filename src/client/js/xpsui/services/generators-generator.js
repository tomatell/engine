				...
				for (var r=1; r<= max_round; r++) {	// r is the round number.
					...
					while (1) {	 //Must get out of this loop with break
						...
						for (var x = 0 ; x < repeat ; x++) {
							...
							var match={home:pHome.team ,visitors:pVisitors.team ,boar:i ,matchNumber:matchPrefix, homeClub:pHome.club ,visitorClub:pVisitors.club};
							...
							if (match.home && match.home.refData) {
								if (match.home.refData.name==='volno') {
									if (baseDataCP.matchIDForNotPlayingTeam==='FALSE' && baseDataCP.typeOfCompetition==='BERGER_SAME_PAIR_TWICE' && x===1) {
										match_id--;
									} else if (baseDataCP.matchIDForNotPlayingTeam==='FALSE' && baseDataCP.typeOfCompetition==='BERGER_BASE' && x===0) {
										match_id--;
									}
								}
							} 
							
							if (match.visitors && match.visitors.refData) {
								if (match.visitors.refData.name==='volno') {
									if (baseDataCP.matchIDForNotPlayingTeam==='FALSE' && baseDataCP.typeOfCompetition==='BERGER_SAME_PAIR_TWICE' && x===1) {
										match_id--;
									} else if (baseDataCP.matchIDForNotPlayingTeam==='FALSE' && baseDataCP.typeOfCompetition==='BERGER_BASE' && x===0) {
										match_id--;
							...
							// don't match_id++ if complement===true
							if (pHome.complement) {
